...
void CQDlgModelThumbnail::initialize()
{
	...
	App::ct->objCont->addObjectToScene(rs,false);
	int res[2]={256,256};
...
void CQDlgModelThumbnail::actualizeBitmap()
{
	...
	for (int i=0;i<int(sel.size());i++)
	{
		...
		if ( (!it->isObjectPartOfInvisibleModel())&&(it->layer&App::ct->mainSettings->activeLayers)&&display )
		{
		...
		for (float y=-1.0f;y<2.0f;y+=2.0f)
		{
			for (float z=-1.0f;z<2.0f;z+=2.0f)
			{
				...
				if (fabs(w(1))>maxPerpY)
					maxPerpY=fabs(w(1));
				float a=fabs(w(0))/tan(THUMBNAIL_THING_VIEW_ANGLE*degToRad*0.5f);
				float b=fabs(w(1))/tan(THUMBNAIL_THING_VIEW_ANGLE*degToRad*0.5f);
...
//	const float divider=centerWeight+4.0f*horizWeight+4.0f*diagWeight;
	for (int i=0;i<128;i++)
	{
		for (int j=0;j<128;j++)
		{
...
void CQDlgModelThumbnail::on_qqFromFile_clicked()
{
	...
	if (filenameAndPath.length()!=0)
	{
		if (VFile::doesFileExist(filenameAndPath))
		{
			...
			if (data==NULL)
				App::uiThread->messageBox_critical(App::mainWindow,strTranslate("Thumbnail"),strTranslate(IDS_TEXTURE_FILE_COULD_NOT_BE_LOADED),VMessageBox::OKELI);
			else
			{
				if ( (resX!=128)||(resY!=128) )
					App::uiThread->messageBox_critical(App::mainWindow,strTranslate("Thumbnail"),strTranslate(IDS_THUMBNAILS_REQUIRE_128X128_RESOLUTION),VMessageBox::OKELI);
				else
				{
