...
void CikGroup::performGroupLoadingMapping(std::vector<int>* map)
{
	for (int i=0;i<int(ikElements.size());i++)
		ikElements[i]->performGroupLoadingMapping(map);
...
bool CikGroup::announceGroupWillBeErased(int groupID,bool copyBuffer)
{ // Return value true means that this object should be destroyed
	...
	{
		if (ikElements[i]->announceGroupWillBeErased(groupID))
		{
...
{ // Will return registered avoidance partners. Partners are stored in the returned
	...
	if (_avoidanceRobotEntity<SIM_IDSTART_COLLECTION)
	{
		...
		if ( (it!=NULL)&&(it->getMainProperty()&sim_objectspecialproperty_measurable) ) // we check the measurable property here (special!!)
			gr1.push_back(it);
	}
	else
		App::ct->collections->getShapesAndDummiesFromGroup(_avoidanceRobotEntity,&gr1,sim_objectspecialproperty_measurable,false);
	if (_avoidanceObstacleEntity<SIM_IDSTART_COLLECTION)
	{
		...
		{ // Here we need to get all objects that are measurable but not contained in gr1!
			App::ct->objCont->getAllShapesAndDummiesFromSceneExcept(gr1,gr2,sim_objectspecialproperty_measurable,false);
		}
	}
	else
		App::ct->collections->getShapesAndDummiesFromGroup(_avoidanceObstacleEntity,&gr2,sim_objectspecialproperty_measurable,false);
...
{ // Return value true means that this object should be destroyed
	...
	while (i<int(ikElements.size()))
	{
		...
		{
			return(true); // new since 9/3/2014
			removeIkElement(ikElements[i]->getObjectID());
			i=0; // ordering may have changed
		}
		else
	...
	*/
	...
	// Following to replace above code. New since 9/3/2014
	for (int i=0;i<int(ikElements.size());i++)
	{
		if (ikElements[i]->announceObjectWillBeErased(objID,copyBuffer))
			return(true);
	...
	for (int elementNumber=0;elementNumber<int(ikElements.size());elementNumber++)
	{
		...
		{ // We add all joint between tooltip and base which are not yet present:
			...
			while ( (iterat!=base)&&(iterat!=NULL) )
			{
				if (iterat->getObjectType()==sim_object_joint_type)
				{
					// FOLLOWING JUST FOR V-REP (FASTER, BUT USING BOOST LIB):
...
{ // Return value is one of following: sim_ikresult_not_performed, sim_ikresult_success, sim_ikresult_fail
	...
	if (!forMotionPlanning)
		App::ct->infoDisplay->inverseKinematicsStart();
	...
	{
		if (!forMotionPlanning)
			App::ct->infoDisplay->inverseKinematicsEnd();
...
{	// Return value -1 means that an error occured --> keep old configuration
	...
	if ( (constraints&sim_ik_avoidance_constraint)&&(_avoidanceRobotEntity!=-1)&&(_avoidanceObstacleEntity!=-1) )
	{
	...
	for (int i=0;i<int(allJoints.size());i++)
	{
		...
		{
			if (allJoints[i]->getJointMode()==sim_jointmode_ikdependent)
				numberOfRows++;
		}
	}
	//---------------------------------------------------------------------------
	...
	for (int i=0;i<int(allJoints.size());i++)
	{
		if ( (allJoints[i]->getJointMode()==sim_jointmode_ikdependent)&&(allJoints[i]->getJointType()!=sim_joint_spherical_subtype) )
		{
	...
	{
		delete _lastJacobian;
		_lastJacobian=new CMatrix(mainMatrix);
	}
...
float CikGroup::getDeterminant(CMatrix& m,std::vector<int>* activeRows,std::vector<int>* activeColumns)
{ // activeRows and activeColumns are NULL by default (--> all rows anc columns are active)
	...
	{ // We compute this directly:
		return(m(activeRows->at(0),activeColumns->at(0))*m(activeRows->at(1),activeColumns->at(1))-
			m(activeRows->at(0),activeColumns->at(1))*m(activeRows->at(1),activeColumns->at(0)) );
	}
	else
	{ // The general routine
		std::vector<int> actR;
...
/*
		int marc=activeRows->size();
		marc=activeColumns->size();
		for (int k=0;k<activeRows->size();k++)
			marc=activeRows->at(k);
		for (k=0;k<activeColumns->size();k++)
			marc=activeColumns->at(k);
*/
...
//			retVal+=m(i,activeColumns->at(0))*getDeterminant(m,&actR,&actC)*pow(-1.0f,(double)(i+1));
			retVal+=m(i,activeColumns->at(0))*getDeterminant(m,&actR,&actC)*pow(float(-1.0f),float(rowInd+1));
		}
		return(retVal);
	}
}
