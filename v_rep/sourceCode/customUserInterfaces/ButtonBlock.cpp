...
void CButtonBlock::displayBlock(int winSize[2],bool justCameToFront)
{
	...
	int psClosed[2]={blockWidth*(buttonWidth+2),blockHeight*(buttonHeight+2)};
	int psOpen[2]={blockWidth*(buttonWidth+2),blockHeight*(buttonHeight+2)};
	int vs[2]={0,psOpen[1]};
	...
	{
		psClosed[0]=((rollupMax.x-rollupMin.x)+1)*(buttonWidth+2);
		psClosed[1]=((rollupMax.y-rollupMin.y)+1)*(buttonHeight+2);
		vs[0]=rollupMin.x*(buttonWidth+2);
		vs[1]=(blockHeight-rollupMin.y)*(buttonHeight+2);
	}
	int aa;
	...
	VPoint buttonSize(buttonWidth,buttonHeight);
	VPoint pos;
	VPoint blockPos;
	getBlockPositionAbsolute(blockPos);

	// Following is needed to see the shift-selection:
	VPoint startP(0,0); // initialization to (0,0) not needed, but to avoid compiler complaints
	VPoint endP(0,0); // initialization to (0,0) not needed, but to avoid compiler complaints
	if ( (App::ct->buttonBlockContainer->getBlockInEdition()==blockID)&&(App::ct->buttonBlockContainer->editMode) )
	{
	...
	if ( (App::ct->buttonBlockContainer->getBlockInEdition()==blockID)&&(App::ct->buttonBlockContainer->editMode) )
	{
		VPoint blockSize((buttonWidth+2)*blockWidth-2,(buttonHeight+2)*blockHeight-2);
		VPoint otherButtonSize(buttonWidth,buttonHeight);
...
int CButtonBlock::mouseDownCatch(int xCoord,int yCoord,bool& cursorCatch,bool test)
{
	...
	for (int i=allButtons.size()-1;i>=0;i--)
	{
		...
		if ( isButtonInVisibleZone(itButton)&&
			...
			(App::ct->buttonBlockContainer->getButtonEditMode()&&((_attributes&sim_ui_property_systemblock)==0)) ) ) // The block might be rolled-up!
		{
...
void CButtonBlock::setButtonSize(VPoint theSize)
{
	buttonWidth=theSize.x;
	buttonHeight=theSize.y;
	tt::limitValue(BUTTON_MIN_X_SIZE,10000,buttonWidth);
	buttonWidth=(buttonWidth/2)*2;		// has to be a multiple of 2
	buttonHeight=(buttonHeight/2)*2;	// has to be a multiple of 2
	tt::limitValue(BUTTON_MIN_Y_SIZE,10000,buttonHeight);
}

CButtonBlock* CButtonBlock::copyYourself()
{
	CButtonBlock* newBlock=new CButtonBlock(blockWidth,blockHeight,buttonWidth,buttonHeight,viewToAppearOn);
	...
	newBlock->desiredBlockPosition.x=blockPosition.x+_copyPositionOffset;
	newBlock->desiredBlockPosition.y=blockPosition.y+_copyPositionOffset;
	newBlock->blockPosition.x=blockPosition.x+_copyPositionOffset;
	newBlock->blockPosition.y=blockPosition.y+_copyPositionOffset;
...
void CButtonBlock::getButtonSize(VPoint& theSize)
{
	theSize.x=buttonWidth;
	theSize.y=buttonHeight;
}
...
{	// Same as insert button, with the difference that we don't check if
	...
	{
		for (int j=0;j<theNewButton->getHeight();j++)
			positionFastIndex[theNewButton->xPos+i+(theNewButton->yPos+j)*blockWidth]=allButtons.size()-1;
	}
...
{
	VPoint pos;
	blockSize.x=(buttonWidth+2)*blockWidth-2;
	blockSize.y=(buttonHeight+2)*blockHeight-2;
	otherButtonSize.x=(buttonWidth+2)*button->getLength()-2;
	otherButtonSize.y=(buttonHeight+2)*button->getHeight()-2;
	if (_attributes&sim_ui_property_relativetotopborder)
