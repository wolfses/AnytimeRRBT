...
{	// This is the vector version of the routine!
	...
	{
	public: 
		CTriangle(int aa,int bb,int cc,int ind)
		{
			a=aa;
			b=bb;
			c=cc;
	...
	// We put all triangles in an array. Every triangle is stored 3 times under
	// each of its a, b or c-value index
  
	std::vector<void*> allTriangles(vertices->size()/3,NULL);
	for (int i=0;i<int(indices->size())/3;i++)
	{
		int pt[3];
		pt[0]=indices->at(3*i+0);
		pt[1]=indices->at(3*i+1);
		pt[2]=indices->at(3*i+2);
		for (int j=0;j<3;j++)
		{
			CTriangle* el=new CTriangle(pt[0],pt[1],pt[2],i);
			if (allTriangles[pt[j]]==NULL)
				allTriangles[pt[j]]=el;
			else
				((CTriangle*)allTriangles[pt[j]])->add(el);
		}
	}
	// Now we just iteratively (less memory) explore linked triangles starting with the first one
	std::vector<BYTE> explorationState(indices->size()/3,0);
	int i=0;
	while (allTriangles[i]==NULL)
		i++;
	std::vector<void*> exTri;
	exTri.reserve(vertices->size());
	exTri.clear();
	CTriangle* it=(CTriangle*)allTriangles[i];
	int oneBit=0;
	int twoBits=0;
	while (it!=NULL)
	{
		exTri.push_back(it);
		if (it->a==i) 
			_setExtractionExploration(&explorationState,it->index,1,oneBit,twoBits);
		if (it->b==i) 
			_setExtractionExploration(&explorationState,it->index,2,oneBit,twoBits);
		if (it->c==i) 
			_setExtractionExploration(&explorationState,it->index,4,oneBit,twoBits);
		it=it->next;
	}
	while (exTri.size()!=0)
	{
		CTriangle* el=(CTriangle*)exTri.at(exTri.size()-1);
		exTri.erase(exTri.end()-1);
		if ((explorationState[el->index]&1)==0)
		{ // We explore the first point
			_setExtractionExploration(&explorationState,el->index,1,oneBit,twoBits);
			it=(CTriangle*)allTriangles[el->a];
			while (it!=NULL)
			{
				if (explorationState[it->index]!=7) 
					exTri.push_back(it);
				if (it->a==el->a) 
					_setExtractionExploration(&explorationState,it->index,1,oneBit,twoBits);
				if (it->b==el->a) 
		...
		if ((explorationState[el->index]&2)==0)
		{ // We explore the second point
			_setExtractionExploration(&explorationState,el->index,2,oneBit,twoBits);
			it=(CTriangle*)allTriangles[el->b];
			while (it!=NULL)
			{
				if (explorationState[it->index]!=7) 
					exTri.push_back(it);
		...
		if ((explorationState[el->index]&4)==0)
		{ // We explore the third point
			_setExtractionExploration(&explorationState,el->index,4,oneBit,twoBits);
			it=(CTriangle*)allTriangles[el->c];
			while (it!=NULL)
			{
				if (explorationState[it->index]!=7) 
					exTri.push_back(it);
	...
	// Now all triangles belonging to the same shape are marked
	// We first free allocated memory
	...
	// Now we build the indices of the shape which will be extracted
	int counter=0;
	for (int i=0;i<int(explorationState.size());i++)
		if (explorationState[i]!=0)
			counter++;
	sIndices->reserve(3*counter);
	sIndices->clear();
	for (int i=0;i<int(explorationState.size());i++)
	{
		if (explorationState[i]!=0)
	...
	// Now we need only the referenced vertices in sVertices:
	useOnlyReferencedVertices(vertices,sVertices,sIndices);

	// Now we build the indices of the shape which will not be extracted
	std::vector<int> tmpIndices(*indices);
	indices->clear();
	for (int i=0;i<int(explorationState.size());i++)
		if (explorationState[i]==0)
		{
			indices->push_back(tmpIndices[3*i+0]);
			indices->push_back(tmpIndices[3*i+1]);
			indices->push_back(tmpIndices[3*i+2]);
		}
	tmpIndices.clear();
	// Now we need only the referenced vertices in vertices:
	std::vector<float> tmpVertices(*vertices);
	vertices->clear();	
	useOnlyReferencedVertices(&tmpVertices,vertices,indices);
	return(indices->size()!=0);
}

void CMeshManip::_setExtractionExploration(std::vector<BYTE>* exploration,int index,BYTE bit,
								  int &allBits,int &twoBits)
{ // This is the vector version of the routine!
	// Dont't use this function. It is a helper function for "extractOneShape".
	// It sets the exploration bits and counts the number of 'one bit set'
	// and 'two bits set'
	BYTE a=exploration->at(index);
	if ((a==3)||(a==5)||(a==6))
	{	// Two bits are already set.
		if ((a|bit)==7)
		{
			twoBits--;
			allBits++;
			exploration->at(index)=7;
		}
	}
	else if (a!=0)
	{
		...
		{
			twoBits++;
			exploration->at(index)=a|bit;
		}
	}
	else
		exploration->at(index)=bit;
}
...
/*
void CMeshManip::removeDoubleVertices(std::vector<float>& vertices,std::vector<int>& mapping,
				float tolerance)
{	// Here we remove vertices which are identical within a certain tolerance.
	// If a vertex is a duplicate, the indices pointing onto it will be remapped.
//	printf("bla");
...
//		printf("%i, %i, %i, %i, %i, %i, %i, %i\n",mxmymz.size(),mxmypz.size(),mxpymz.size(),mxpypz.size(),pxmymz.size(),pxmypz.size(),pxpymz.size(),pxpypz.size());
		...
		// Now we just need to check for duplicates in the border:
...
{	// First create an edge list
	...
	std::vector<BYTE> exploredState(indices->size()/3,0);
