...
CConvexVolume* CConvexVolume::copyYourself()
{
	...
	newConvexVolume->firstVolumeCorners.insert(newConvexVolume->firstVolumeCorners.end(),firstVolumeCorners.begin(),firstVolumeCorners.end());
...
void CConvexVolume::serialize(CSer& ar)
{
	...
	{		// Storing
		...
		ar.storeDataName("Nc6");
		ar << int(firstVolumeCorners.size());
		for (int i=0;i<int(firstVolumeCorners.size());i++)
			ar << firstVolumeCorners[i](0) << firstVolumeCorners[i](1) << firstVolumeCorners[i](2);
		ar.flush();
		//***************************************************************************
		...
		while (theName.compare(SER_END_OF_OBJECT)!=0)
		{
			...
			if (theName.compare(SER_END_OF_OBJECT)!=0)
			{
				...
				if (theName.compare("Nc6")==0)
				{
					...
					int vectSize;
					ar >> vectSize;
					firstVolumeCorners.clear();
					C3Vector dum;
					for (int i=0;i<vectSize;i++)
					{
						ar >> dum(0) >> dum(1) >> dum(2);
						firstVolumeCorners.push_back(dum);
					}
				}
				//***************************************************************************
...
bool CConvexVolume::getVolumeBoundingBox(C3Vector& minV,C3Vector& maxV)
{ 
...
{
	tt::limitValue(0.001f*degToRad,piValTimes2,theAngle);
	if ( (_volumeType==DISC_TYPE_CONVEX_VOLUME)||(_volumeType==CONE_TYPE_CONVEX_VOLUME) )
	{
		if ( (theAngle>piValue)&&(theAngle<piValue*1.01f) )
			theAngle=piValue;
		if (theAngle>=piValue*1.01f)
			theAngle=piValTimes2;
	}
	angle=theAngle;
	solveInterferences();
	computeVolumes();
...
void CConvexVolume::computeVolumeEdges(std::vector<float>& edges)
{
	...
	if (_volumeType==CYLINDER_TYPE_CONVEX_VOLUME)
	{
		...
		for (int i=0;i<faceNumber;i++)
		{
			...
			add3Values(edges,m,+cos(alpha)*radiusFar,+sin(alpha)*radiusFar,offset+range);
			add3Values(edges,m,+cos(alpha)*radiusFar,-sin(alpha)*radiusFar,offset+range);
			tr.buildZRotation(piValTimes2/faceNumber);
			m=m*tr;
		}
	}
	if (_volumeType==PYRAMID_TYPE_CONVEX_VOLUME)
...
void CConvexVolume::computeVolumes()
{
	...
	if (_volumeType==DISC_TYPE_CONVEX_VOLUME)
	{
		...
		// The closing part (if angle != piValTimes2)
		if (angle<piValue*1.03f)
		{
...
void CConvexVolume::generateSphereEdges(std::vector<float>& edges,float radius)
{
	int subDivW=12;
	int subDivV=12;
	float dw=piValTimes2/float(subDivW);
