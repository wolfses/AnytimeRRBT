...
int CSimpleFilter::_getFilterTypeAndNameFromHeaderAndID(int header,int id,std::string& name)
{
	for (int i=0;i<int(_externalFilters.size());i++)
	{
		if ((_externalFilters[i].header==header)&&(_externalFilters[i].id==id))
		{
			...
			return(simple_filter_customized+i);
		}
	}
	return(-1); // not found!
}
...
void CSimpleFilter::getCustomFilterParameters(std::vector<BYTE>& params)
{
	params.assign(_customFilterParameters.begin(),_customFilterParameters.end());
}

void CSimpleFilter::setCustomFilterParameters(const std::vector<BYTE>& params)
{
...
{ // Is called during loading!! (keep simple)
	...
	if (_filterType>=simple_filter_customized)
	{
		int ind=_filterType-simple_filter_customized;
	...
	if (_filterType==simple_filter_5x5filter)
	{
		_intParameters.push_back(1); // number of passes
		// We calculate a Gaussian blur:
	...
	if (_filterType==simple_filter_keeporremovecolors)
	{
	...
	if (_filterType==simple_filter_scaleandoffsetcolors)
	{
	...
	if (_filterType==simple_filter_correlationwithbuffer1)
	{
	...
	if (_filterType==simple_filter_shift)
	{
	...
	if (_filterType==simple_filter_circularcut)
	{
	...
	if (_filterType==simple_filter_binary)
	{
...
void CSimpleFilter::serialize(CSer& ar)
{
	...
	{		// Loading
		...
		if ((_filterType<0)||(_filterType>=simple_filter_customized))
			_filterType=_getFilterTypeAndNameFromHeaderAndID(_customFilterHeader,_customFilterID,_customFilterName);
	}
}
...
{
	if (filterType==simple_filter_originalimage)
		return(IDS_FILTER_ORIGINAL_IMAGE_TO_WORK_IMAGE);
	if (filterType==simple_filter_originaldepth)
		return(IDS_FILTER_ORIGINAL_DEPTH_IMAGE_TO_WORK_IMAGE);
	if (filterType==simple_filter_uniformimage)
		return(IDS_FILTER_UNIFORM_IMAGE_TO_WORK_IMAGE);
	if (filterType==simple_filter_tooutput)
		return(IDS_FILTER_WORK_IMAGE_TO_OUTPUT_IMAGE);
	if (filterType==simple_filter_tobuffer1)
		return(IDS_FILTER_WORK_IMAGE_TO_BUFFER1);
	if (filterType==simple_filter_tobuffer2)
		return(IDS_FILTER_WORK_IMAGE_TO_BUFFER2);
	if (filterType==simple_filter_frombuffer1)
		return(IDS_FILTER_BUFFER1_TO_WORK_IMAGE);
	if (filterType==simple_filter_frombuffer2)
		return(IDS_FILTER_BUFFER2_TO_WORK_IMAGE);

	if (filterType==simple_filter_swapwithbuffer1)
		return(IDS_FILTER_SWAP_WORK_WITH_BUFFER1);
	if (filterType==simple_filter_addtobuffer1)
		return(IDS_FILTER_ADD_WORK_TO_BUFFER1);
	if (filterType==simple_filter_subtractfrombuffer1)
		return(IDS_FILTER_SUBTRACT_WORK_FROM_BUFFER1);

	if (filterType==simple_filter_swapbuffers)
		return(IDS_SWAP_BUFFER1_WITH_BUFFER2);
	if (filterType==simple_filter_addbuffer1)
		return(IDS_FILTER_ADD_BUFFER1_TO_WORK_IMAGE);
	if (filterType==simple_filter_subtractbuffer1)
		return(IDS_FILTER_SUBTRACT_BUFFER1_FROM_WORK_IMAGE);
	if (filterType==simple_filter_multiplywithbuffer1)
		return(IDS_FILTER_MULTIPLY_WORK_IMAGE_WITH_BUFFER1);
	if (filterType==simple_filter_horizontalflip)
		return(IDS_FILTER_FLIP_WORK_IMAGE_HORIZONTALLY);
	if (filterType==simple_filter_verticalflip)
		return(IDS_FILTER_FLIP_WORK_IMAGE_VERTICALLY);
	if (filterType==simple_filter_3x3filter)
		return(IDS_FILTER_3X3FILTER_WORK_IMAGE);
	if (filterType==simple_filter_5x5filter)
		return(IDS_FILTER_5X5FILTER_WORK_IMAGE);
	if (filterType==simple_filter_rectangularcut)
		return(IDS_FILTER_RECTANGULAR_CUT_WORK_IMAGE);
	if (filterType==simple_filter_normalize)
		return(IDS_FILTER_NORMALIZE_WORK_IMAGE);
	if (filterType==simple_filter_colorsegmentation)
		return(IDS_COLOR_SEGMENT_WORK_IMAGE);
	if (filterType==simple_filter_blobextraction)
		return(IDS_BLOB_DETECTION_WORK_IMAGE);
	if (filterType==simple_filter_imagetocoord)
		return(IDS_DEPTH_TO_COORD_WORK_IMAGE);
	if (filterType==simple_filter_pixelchange)
		return(IDS_PIXEL_CHANGE_WORK_IMAGE);	
	if (filterType==simple_filter_intensityscale)
		return(IDS_FILTER_INTENSITY_SCALE_WORK_IMAGE);
	if (filterType==simple_filter_keeporremovecolors)
		return(IDS_FILTER_SELECTIVE_COLORS_WORK_IMAGE);
	if (filterType==simple_filter_scaleandoffsetcolors)
		return(IDS_FILTER_OFFSET_AND_SCALE_COLORS_WORK_IMAGE);
	if (filterType==simple_filter_correlationwithbuffer1)
		return(IDS_FILTER_BUFFER1_AND_WORK_IMAGE_CORRELATION);
	if (filterType==simple_filter_binary)
		return(IDS_FILTER_BINARY_WORK_IMAGE);
	if (filterType==simple_filter_sharpen)
		return(IDS_FILTER_SHARPEN_WORK_IMAGE);
	if (filterType==simple_filter_edge)
		return(IDS_FILTER_EDGE_DETECTION_ON_WORK_IMAGE);
	if (filterType==simple_filter_shift)
		return(IDS_FILTER_REPOSITION_WORK_IMAGE);
	if (filterType==simple_filter_circularcut)
		return(IDS_FILTER_CIRCULAR_CUT_WORK_IMAGE);
	if (filterType==simple_filter_resize)
		return(IDS_FILTER_RESIZE_WORK_IMAGE);
	if (filterType==simple_filter_rotate)
		return(IDS_FILTER_ROTATE_WORK_IMAGE);
	if (filterType>=simple_filter_customized)
	{
		std::string retString("EXT: ");
		int ind=filterType-simple_filter_customized;
		if (ind>=int(_externalFilters.size()))
...
std::string CSimpleFilter::getFilterString()
{
	...
	if ( (retString.length()==0)&&((_filterType>=simple_filter_customized)||(_filterType==-1) ) )
...
int CSimpleFilter::getAvailableFilter(int filterTypeIndex)
{
	...
	if (filterTypeIndex==i++)
		return(simple_filter_originalimage);
	if (filterTypeIndex==i++)
		return(simple_filter_originaldepth);
	if (filterTypeIndex==i++)
		return(simple_filter_uniformimage);

	if (filterTypeIndex==i++)
		return(simple_filter_tooutput);
	if (filterTypeIndex==i++)
		return(simple_filter_tobuffer1);
	if (filterTypeIndex==i++)
		return(simple_filter_tobuffer2);

	if (filterTypeIndex==i++)
		return(simple_filter_frombuffer1);
	if (filterTypeIndex==i++)
		return(simple_filter_frombuffer2);


	if (filterTypeIndex==i++)
		return(simple_filter_swapwithbuffer1);
	if (filterTypeIndex==i++)
		return(simple_filter_swapbuffers);

	if (filterTypeIndex==i++)
		return(simple_filter_addbuffer1);
	if (filterTypeIndex==i++)
		return(simple_filter_addtobuffer1);

	if (filterTypeIndex==i++)
		return(simple_filter_subtractbuffer1);
	if (filterTypeIndex==i++)
		return(simple_filter_subtractfrombuffer1);
	if (filterTypeIndex==i++)
		return(simple_filter_multiplywithbuffer1);

	if (filterTypeIndex==i++)
		return(simple_filter_horizontalflip);
	if (filterTypeIndex==i++)
		return(simple_filter_verticalflip);
	if (filterTypeIndex==i++)
		return(simple_filter_rotate);
	if (filterTypeIndex==i++)
		return(simple_filter_shift);
	if (filterTypeIndex==i++)
		return(simple_filter_resize);

	if (filterTypeIndex==i++)
		return(simple_filter_3x3filter);
	if (filterTypeIndex==i++)
		return(simple_filter_5x5filter);
	if (filterTypeIndex==i++)
		return(simple_filter_sharpen);
	if (filterTypeIndex==i++)
		return(simple_filter_edge);
	if (filterTypeIndex==i++)
		return(simple_filter_correlationwithbuffer1);
	if (filterTypeIndex==i++)
		return(simple_filter_rectangularcut);
	if (filterTypeIndex==i++)
		return(simple_filter_circularcut);

	if (filterTypeIndex==i++)
		return(simple_filter_normalize);
	if (filterTypeIndex==i++)
		return(simple_filter_colorsegmentation);
	if (filterTypeIndex==i++)
		return(simple_filter_intensityscale);
	if (filterTypeIndex==i++)
		return(simple_filter_keeporremovecolors);
	if (filterTypeIndex==i++)
		return(simple_filter_scaleandoffsetcolors);

	if (filterTypeIndex==i++)
		return(simple_filter_blobextraction);
	if (filterTypeIndex==i++)
		return(simple_filter_binary);
	if (filterTypeIndex==i++)
		return(simple_filter_imagetocoord);
	if (filterTypeIndex==i++)
		return(simple_filter_pixelchange);	

	// External filters now:
	for (int j=0;j<int(_externalFilters.size());j++)
	{
		if (filterTypeIndex==i++)
			return(simple_filter_customized+j);
	}
	return(-1);
}
...
bool CSimpleFilter::processAndTrigger(CVisionSensor* sensor,int sizeX,int sizeY,const float* inputImage,const float* inputDepth,float* outputImage,float* workImage,std::vector<float>& returnData,float* buffer1,float* buffer2,CDrawingContainer2D& drawingContainer)
{
	if (!_enabled)
		return(false);
	if (_filterType==simple_filter_originalimage)
	...
	if (_filterType==simple_filter_originaldepth)
	...
	if (_filterType==simple_filter_tooutput)
	...
	if (_filterType==simple_filter_tobuffer1)
	...
	if (_filterType==simple_filter_tobuffer2)
	...
	if (_filterType==simple_filter_frombuffer1)
	...
	if (_filterType==simple_filter_frombuffer2)
	...
	if (_filterType==simple_filter_swapwithbuffer1)
	...
	if (_filterType==simple_filter_addtobuffer1)
	...
	if (_filterType==simple_filter_subtractfrombuffer1)
	...
	if (_filterType==simple_filter_swapbuffers)
	...
	if (_filterType==simple_filter_addbuffer1)
	...
	if (_filterType==simple_filter_subtractbuffer1)
	...
	if (_filterType==simple_filter_multiplywithbuffer1)
	...
	if (_filterType==simple_filter_horizontalflip)
	...
	if (_filterType==simple_filter_verticalflip)
	...
	if (_filterType==simple_filter_normalize)
	...
	if (_filterType==simple_filter_sharpen)
	...
	if ((_filterType>=simple_filter_customized)||(_filterType==-1))
	...
	{ // we might have to trigger!
		...
		{ // within proportions
			...
			{ // within cm x-pos
				...
				{ // within cm y-pos
					float d=fabs(tt::getCyclicAngleDifference(_floatParameters[10],angle,piValue));
...
bool CSimpleFilter::processAndTrigger_blobExtraction(int sizeX,int sizeY,const float* inputImage,const float* inputDepth,float* outputImage,float* workImage,std::vector<float>& returnData,float* buffer1,float* buffer2,CDrawingContainer2D& drawingContainer)
{
	...
	{ // For each blob...
		...
		if (vertices[i]->size()!=0)
		{
			...
			returnData.push_back(blobData[BLOBDATSIZE*i+0]/float(sizeX*sizeY)); // relative size of the blob
