...
int CGraphData::getMovingAverageCount()
{
...
void CGraphData::setDerivativeIntegralAndCumulative(int val)
{
	...
	resetData(_floatData.size());
}

int CGraphData::getDerivativeIntegralAndCumulative()
{
	return(_derivativeIntegralAndCumulative);
}

bool CGraphData::getLabel()
{ 
	return(label);
}

bool CGraphData::getLinkPoints()
{
	return(linkPoints);
}

int CGraphData::getIdentifier()
{
	return(identifier);
}

int CGraphData::getDataType()
{
	return(dataType);
}
int CGraphData::getDataObjectID()
{
	return(dataObjectID);
}
int CGraphData::getDataObjectAuxID()
{
	return(dataObjectAuxID);
}

float CGraphData::getZoomFactor()
{
	return(zoomFactor);
}

float CGraphData::getAddCoeff()
{
	return(addCoeff);
}

bool CGraphData::getVisible()
{
	return(visible);
}
			...
			else
			{
				...
				{ // previous data was valid
					if (!cyclic)
						...
						_transformedFloatData[absIndex]=(tt::getCyclicAngleDifference(_floatData[absIndex],_floatData[prevIndex],range))/dt;
					_transformedFloatDataValidFlags[absIndex/8]|=(1<<(absIndex&7)); // valid data
				}
				else
			...
			{ // previous data was valid
				...
				{ // previous transformed data was valid
					if (!cyclic)
						...
						_transformedFloatData[absIndex]=_transformedFloatData[prevIndex]+(_floatData[prevIndex]+tt::getCyclicAngleDifference(_floatData[absIndex],_floatData[prevIndex],range)*0.5f)*dt;
				}
				else
			...
			{ // previous data was valid
				...
				{ // previous transformed data was valid
					if (!cyclic)
						...
						_transformedFloatData[absIndex]=_transformedFloatData[prevIndex]+(_floatData[prevIndex]+tt::getCyclicAngleDifference(_floatData[absIndex],_floatData[prevIndex],range));
				}
				else
...
bool CGraphData::getValue(int absIndex,float& v)
{
...
bool CGraphData::getValueRaw(int absIndex,float& v)
{ // isValid can be null;
