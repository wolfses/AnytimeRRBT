...
void CGraphingRoutines::adjustDataToUserMetric(CGraphData* it,float& v,int derivativeIntegralOrCumulative)
{
	int dType=it->getDataType();
	if ( (dType==GRAPH_3DOBJECT_ALL_X_ABS)||
		(dType==GRAPH_3DOBJECT_ALL_Y_ABS)||
	...
	if ( (dType==GRAPH_3DOBJECT_ALL_XVEL_ABS)||
		...
		(dType==GRAPH_3DOBJECT_ALL_VEL_ABS) )
	{
		v*=gv::meterToUser/gv::secondsToUser;
	}

	if (dType==GRAPH_3DOBJECT_ALL_ANGULAR_VEL_ABS)
	{
		v*=gv::radToUser/gv::secondsToUser;
	}
	...
	if ( (dType>GRAPH_3DOBJECT_JOINT_SUBSTART)&&(dType<GRAPH_3DOBJECT_JOINT_SUBEND) )
	{
		...
		if (dType==GRAPH_3DOBJECT_JOINT_DYN_ORIENTATION_ERROR)
			v*=gv::radToUser;
		if (dType==GRAPH_3DOBJECT_JOINT_DYN_FORCE_OR_TORQUE)
		{	// New since 1/6/2011: We have to check if this is a joint and what type:
			if (act!=NULL)
			{
				if (act->getJointType()==sim_joint_prismatic_subtype)
					v*=gv::kilogramsToUser*gv::meterToUser/(gv::secondsToUser*gv::secondsToUser);
				if (act->getJointType()==sim_joint_revolute_subtype)
					v*=gv::kilogramsToUser*gv::meterToUser*gv::meterToUser/(gv::secondsToUser*gv::secondsToUser);
			}
		}
	}
	if ( (dType>GRAPH_3DOBJECT_FORCESENSOR_SUBSTART)&&(dType<GRAPH_3DOBJECT_FORCESENSOR_SUBEND) )
	...
	if ( (dType==GRAPH_3DOBJECT_MILL_CUT_SURFACE)||
		(dType==GRAPH_NOOBJECT_MILL_OVERALL_CUT_SURFACE) )
	{
		v*=gv::meterToUser*gv::meterToUser;
	}

	if ( (dType==GRAPH_3DOBJECT_MILL_CUT_VOLUME)||
		(dType==GRAPH_NOOBJECT_MILL_OVERALL_CUT_VOLUME) )
	{
		v*=gv::meterToUser*gv::meterToUser*gv::meterToUser;
	}
	...
	if (derivativeIntegralOrCumulative==DATA_STREAM_DERIVATIVE)
		v/=gv::secondsToUser;
	if (derivativeIntegralOrCumulative==DATA_STREAM_INTEGRAL)
		v*=gv::secondsToUser;
}

void CGraphingRoutines::getCyclicAndRangeValues(CGraphData* it,bool& cyclic,float& range)
{
	int dType=it->getDataType();
...
bool CGraphingRoutines::getDataName(int objectType,int dataIndex,std::string& dataName)
...
bool CGraphingRoutines::getDataValue(int objectType,int dataIndex,int objectID,float& value,const C7Vector* graphCTM)
{	// If return value is true, value contains the asked value (used by graphs)
	...
	if ( (dataIndex>GRAPH_NOOBJECT_START)&&(dataIndex<GRAPH_NOOBJECT_END) )
	{
		if (dataIndex==GRAPH_NOOBJECT_IK_OVERALL_CALCULATION_TIME)
		{
			value=App::ct->infoDisplay->getIkCalculationTime();
			return(true);
		}
		if (dataIndex==GRAPH_NOOBJECT_DYNAMICS_OVERALL_CALCULATION_TIME)
		{
			value=App::ct->infoDisplay->getDynamicsCalculationTime();
			return(true);
		}
		if (dataIndex==GRAPH_NOOBJECT_GCS_OVERALL_CALCULATION_TIME)
		{
			value=App::ct->infoDisplay->getGcsCalculationTime();
			return(true);
		}
		if (dataIndex==GRAPH_NOOBJECT_MAINSCRIPT_EXECUTION_TIME)
		{
			value=App::ct->infoDisplay->getMainScriptExecutionTime();
			return(true);
		}
		if (dataIndex==GRAPH_NOOBJECT_RENDERING_TIME)
		{
			value=App::ct->infoDisplay->getRenderingDuration();
			return(true);
		}
		if (dataIndex==GRAPH_NOOBJECT_WORK_THREADS_EXECUTION_TIME)
		{
			value=App::ct->infoDisplay->getWorkThreadExecutionDuration();
			return(true);
		}
		if (dataIndex==GRAPH_NOOBJECT_SIMULATIONPASS_EXECUTION_TIME)
		{
			value=App::ct->infoDisplay->getSimulationPassExecutionTime()+App::ct->infoDisplay->getRenderingDuration(); // added second part on 3/4/2013
			return(true);
		}
		if (dataIndex==GRAPH_NOOBJECT_COLLISION_QUANTITY)
		{
			...
			{
				CRegColl* aColl=App::ct->collisions->collisionObjects[i];
				if (aColl!=NULL)
		...
		if (dataIndex==GRAPH_NOOBJECT_COLLISION_OVERALL_CALCULATION_TIME)
		{
			value=App::ct->infoDisplay->getCollisionCalculationTime();
			return(true);
		}
		if (dataIndex==GRAPH_NOOBJECT_DISTANCE_OVERALL_CALCULATION_TIME)
		{
			value=App::ct->infoDisplay->getDistanceCalculationTime();
			return(true);
		}
		if (dataIndex==GRAPH_NOOBJECT_PROXSENSOR_OVERALL_CALCULATION_TIME)
		{
			value=App::ct->infoDisplay->getProximitySensorCalculationTime();
			return(true);
		}
		if (dataIndex==GRAPH_NOOBJECT_VISIONSENSOR_OVERALL_CALCULATION_TIME)
		{
			value=App::ct->infoDisplay->getVisionSensorCalculationTime();
			return(true);
		}
		if (dataIndex==GRAPH_NOOBJECT_MILL_OVERALL_CALCULATION_TIME)
		{
			value=App::ct->infoDisplay->getMillingCalculationTime();
			return(true);
		}
		if ( (dataIndex==GRAPH_NOOBJECT_MILL_OVERALL_CUT_SURFACE)||
	...
	{
		CRegColl* it=App::ct->collisions->getObject(objectID);
		if (it==NULL)
			return(false);
		if (dataIndex==GRAPH_COLLISION_BOOLEAN)
		{
	...
	if ( (dataIndex>GRAPH_3DOBJECT_JOINT_SUBSTART)&&(dataIndex<GRAPH_3DOBJECT_JOINT_SUBEND) )
	{
		...
		{ // New since 1/6/2011
			float dummyVal;
			if (it->getDynamicForceOrTorque(dummyVal))
			{
				value=dummyVal;
				return(true);
			}
	...
	if ( (dataIndex>GRAPH_3DOBJECT_FORCESENSOR_SUBSTART)&&(dataIndex<GRAPH_3DOBJECT_FORCESENSOR_SUBEND) )
	{
		...
		C3Vector forces;
		C3Vector torques;
		bool validForces=it->getDynamicForces(forces);
		bool validTorques=it->getDynamicTorques(torques);
		...
		if (dataIndex==GRAPH_3DOBJECT_FORCESENSOR_POSITION_ERROR)
		{
			value=it->getPositionError();
			return(true);
		}
		if (dataIndex==GRAPH_3DOBJECT_FORCESENSOR_ORIENTATION_ERROR)
		{
			value=it->getOrientationError();
			return(true);
		}
...
bool CGraphingRoutines::getGraphObjectName(int objectType,int dataIndex,int objectID,std::string& objName)
...
bool CGraphingRoutines::getObjectsFromGraphCategory(int objectType,int index,int dataIndex,int& objectID)
{	// Returns the objectID and auxDat for all objects concerned by the category 'dataIndex'
	...
	if ( (dataIndex>GRAPH_COLLISION_START)&&(dataIndex<GRAPH_COLLISION_END) )
	{
		if (index>=int(App::ct->collisions->collisionObjects.size()))
			return(false);
		CRegColl* it=App::ct->collisions->collisionObjects[index];
		if (it==NULL)
			return(false);
		objectID=it->getObjectID();
		return(true);
...
{ // If return value is true, dataName contains the text for the dataIndex (used by graphs)
	const int itemLength=15;
	int objectsToCheck[itemLength]={	sim_object_shape_type,
		...
		sim_object_reserved1, // STRANGE123 where are force sensors in this list???
	...
	{
		if (CGraphingRoutines::getDataName(objectsToCheck[i],dataIndex,dataName))
			return(true);
	}
	return(false);
}
...
{	// If return value is true, value contains the asked value (used by graphs)
	...
	for (int i=0;i<itemLength;i++)
	{
		if (CGraphingRoutines::getDataValue(objectsToCheck[i],dataIndex,objectID,value,graphCTM))
			return(true);
	}
	return(false);
}
...
{ // Returns the name of an object being graphed (name of 3DObject, sensor, collision partn., etc)
	...
	for (int i=0;i<itemLength;i++)
	{
		if (CGraphingRoutines::getGraphObjectName(objectsToCheck[i],dataIndex,objectID,objName))
			return(true);
	}
	return(false);
}
...
{	// Returns the objectID and auxDat for all objects concerned by the category 'dataIndex'
	...
	for (int i=0;i<itemLength;i++)
	{
		if (CGraphingRoutines::getObjectsFromGraphCategory(objectsToCheck[i],index,dataIndex,objectID))
			return(true);
	}
	return(false);
}
