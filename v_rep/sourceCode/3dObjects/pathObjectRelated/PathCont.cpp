...
CPathCont::CPathCont()
{
	...
	_attributes=sim_pathproperty_show_line|sim_pathproperty_show_orientation|sim_pathproperty_automatic_orientation|
				sim_pathproperty_show_position|sim_pathproperty_endpoints_at_zero|sim_pathproperty_infinite_acceleration;

	_lineSize=1;
	_squareSize=0.01f;
	_lineColor.setDefaultValues();
	_lineColor.setColor(0.0f,0.5f,1.0f,AMBIENT_MODE);
...
WORD CPathCont::getPathModifID()
{
	return(_pathModifID);
}

int CPathCont::getLifeID()
{
	if (_lifeID==-1)
...
void CPathCont::_draw(std::vector<CPathPoint*>& ptCont,bool pathEditMode,bool isPath,bool showLine,bool stripplePts,bool showSquare,bool showOrientation,int lineSize,float squareSize,bool pathIsOnlySelectedObject,int objectID)
{
	...
	if (showSquare||(pathEditMode&&isPath))
	{
		if (pathEditMode&&isPath)
		{
			...
			if (App::ct->objCont->editModeBuffer.size()!=0)
				lastSel=App::ct->objCont->editModeBuffer[App::ct->objCont->editModeBuffer.size()-1];
			for (int i=0;i<int(selectedPts.size());i++)
			{ // display path points:
				ogl::setColor(0.0f,0.0f,0.0f,EMISSION_MODE);
				if (!selectedPts[i])
					ogl::setColor(0.0f,0.0f,1.0f,AMBIENT_MODE);
				else
				{
				...
				glLoadName(i);
				if (i==0)
					ogl::displaySphere(squareSize,6,4,false);
				else
					ogl::displayBox(squareSize,squareSize,squareSize,true);
				if (showOrientation)
					ogl::drawReference(squareSize*2.0f);
				glPopMatrix();
			}
		}
		else
		{
			std::vector<int>* selP=App::ct->objCont->getPointerToSelectedPathPointIndices_nonEditMode();
			...
			for (int i=0;i<int(ptCont.size());i++)
			{
				ogl::setColor(0.0f,0.0f,0.0f,EMISSION_MODE);
				if (!selS[i])
					ogl::setColor(0.0f,0.0f,1.0f,AMBIENT_MODE); // point is not selected (not selected in the "non edit mode")
				else
	...
	if (showOrientation&&(!isPath))
	{
		for (int i=0;i<int(ptCont.size());i++)
		{
			...
			C4Vector axis=ptCont[i]->getTransformation().Q.getAngleAndAxis();
			glRotatef(axis(0)*radToDeg,axis(1),axis(2),axis(3));	
			ogl::drawReference(squareSize*0.5f);
			glPopMatrix();
		}
	}
...
bool CPathCont::_getBezierPointsForVirtualDistance(double& l,int& index0,int& index1,bool forwardDirection)
{
	...
	App::uiThread->messageBox_critical(App::mainWindow,strTranslate(IDSNOTR_APPLICATION_ERROR),strTranslate(IDSNOTR_STRANGE_ERROR6),VMessageBox::OKELI);
	return(false);
}
...
void CPathCont::handlePath(CPath* it,float deltaTime)
{
	...
	{ // we should not forget to call _handleAttachedDummies even when the path motion is disabled!!
		handlePath_keepObjectUnchanged(deltaTime,_position,_nominalVelocity,_targetNominalVelocity,_maxAcceleration,(_attributes&sim_pathproperty_invert_velocity)!=0,(_attributes&sim_pathproperty_infinite_acceleration)!=0);
...
CBezierPathPoint* CPathCont::_addBezierPathPoint(const C7Vector& transf,float maxRelAbsVelocity,float onSpotDistance,WORD auxFlags,const float auxChannels[4])
{
...
void CPathCont::_recomputeBezierPathMaxVelocities()
{
	...
	if ( (_attributes&sim_pathproperty_auto_velocity_profile_translation)||
		(_attributes&sim_pathproperty_auto_velocity_profile_rotation) )
	{
	...
	if (_attributes&sim_pathproperty_endpoints_at_zero)
	{
		if ( (_attributes&sim_pathproperty_closed_path)==0 )
		{
	...
	if (_attributes&sim_pathproperty_auto_velocity_profile_rotation)
	{
		for (int i=0;i<int(_bezierPathPoints.size());i++)
		{
			if ( ((_attributes&sim_pathproperty_closed_path)==0)&&(i==int(_bezierPathPoints.size())-1) )
...
void CPathCont::_recomputeBezierPoints()
{
	...
	for (int i=0;i<int(_simplePathPoints.size());i++)
	{
		...
		else
		{
			...
			WORD auxFlags0=itb->getAuxFlags();
			WORD auxFlags1=itm->getAuxFlags();
			WORD auxFlags2=ita->getAuxFlags();
