...
void CGhostObjectContainer::renderYour3DStuff_nonTransparent(CViewableBase* renderingObject,int displayAttrib)
{
	...
	{
		float simulationTime=App::ct->simulation->getSimulationTime();
		float realTime=App::ct->simulation->getSimulationTime_real();
...
void CGhostObjectContainer::newSceneProcedure()
...
int CGhostObjectContainer::addGhost(int theGroupId,int theObjectHandle,int theOptions,float theStartTime,float theEndTime,const float theColor[12])
{
	...
	int nextGhostId=0;
	for (int i=0;i<int(freeSpot.size());i++)
	{
		if (freeSpot[i])
		{
			nextGhostId=i;
	...
	for (int i=0;i<int(objsToAdd.size());i++)
	{
		CShape* obj=objsToAdd[i];
		if (obj->getShouldObjectBeDisplayed(0)||(theOptions&8))
