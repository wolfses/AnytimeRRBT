...
void CPathPlanningTask::setStepSize(float size)
{
	...
	stepSize=size;
	if (sin(44.0f*degToRad)*minTurningCircleDiameter<stepSize)
		minTurningCircleDiameter=stepSize/sin(44.0f*degToRad);
}
...
void CPathPlanningTask::setSearchRange(float searchMin[4],float searchR[4])
{
	...
	searchMinValue[3]=v;
	v=searchR[3];
	tt::limitValue(0.1f*degToRad,piValTimes2,v);
	searchRange[3]=v;
}
...
void CPathPlanningTask::setMinTurningCircleDiameter(float d)
{
	...
	if (sin(44.0f*degToRad)*minTurningCircleDiameter<stepSize)
		stepSize=minTurningCircleDiameter*sin(44.0f*degToRad);
}
...
void CPathPlanningTask::performGroupLoadingMapping(std::vector<int>* map)
{
	if (_robotEntityID>=SIM_IDSTART_COLLECTION)
		_robotEntityID=App::ct->objCont->getLoadingMapping(map,_robotEntityID);
	if (_obstacleEntityID>=SIM_IDSTART_COLLECTION)
...
bool CPathPlanningTask::announceGroupWillBeErased(int groupID,bool copyBuffer)
{ // Return value true means that this object should be destroyed
...
void CPathPlanningTask::renderYour3DStuff()
{
	...
	if (it!=NULL)
	{
		...
		ogl::setColor(0.0f,1.0f,1.0f,EMISSION_MODE);
		glBegin(GL_LINE_STRIP);
		glVertex3fv(corners[0].data);
		glVertex3fv(corners[2].data);
		glVertex3fv(corners[6].data);
		glVertex3fv(corners[4].data);
		glVertex3fv(corners[0].data);
		glVertex3fv(corners[1].data);
		glVertex3fv(corners[3].data);
		glVertex3fv(corners[7].data);
		glVertex3fv(corners[5].data);
		glVertex3fv(corners[1].data);
		glEnd();
		glBegin(GL_LINES);
		glVertex3fv(corners[4].data);
...
bool CPathPlanningTask::initiateSteppedSearch(bool showProgressDlg,float maxTime,float subDt)
{
	...
	if (pathPlanningType==sim_pathplanning_nonholonomic)
	{
		...
		printf("%s - %s : %d \n createNonHolonomicPathPlanningObject\n",__FILE__,__func__,__LINE__);
	...
	{ // This is the last time in this routine. Display the trees and clean-up!
		if ((retVal>0)&&_showSearchTrees)
		{
			...
			if (fromStart!=NULL)
			{
				...
				it->color.setColor(1.0f,0.0f,0.0f,AMBIENT_MODE);
			...
			{
				CDrawingObject* it=new CDrawingObject(sim_drawing_lines,1.0f,0.0f,-1,1000000,false);
				it->color.setColor(0.0f,0.0f,1.0f,AMBIENT_MODE);
